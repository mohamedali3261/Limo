...
export const Layout: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const levelUp = localStorage.getItem('french_level_up');
    if (levelUp === 'true') {
      const level = parseInt(localStorage.getItem('french_new_level') || '0', 10);
      setNewLevel(level);
      setShowLevelUp(true);
      localStorage.removeItem('french_level_up');
      localStorage.removeItem('french_new_level');
    }
  }, [location.pathname]);

  const navItems = [
    { name: 'خريطة المغامرة', path: '/french', icon: Map },
    { name: 'محادثات تفاعلية', path: '/french/pronunciation', icon: Mic },
    { name: 'المحادثة', path: '/french/phrases', icon: BookOpen },
    { name: 'اختبارات', path: '/french/quizzes', icon: Lightbulb },
    { name: 'حسابي', path: '/french/profile', icon: User },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] flex flex-col md:flex-row overflow-hidden font-sans" dir="rtl">
      ...
      {/* Background Decor */}
      <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-blue-500/10 rounded-full blur-[120px] pointer-events-none -z-10" />
      <div className="fixed bottom-0 left-0 w-[500px] h-[500px] bg-blue-500/5 rounded-full blur-[120px] pointer-events-none -z-10" />
      ...
      <nav className="hidden md:flex w-56 bg-white/80 backdrop-blur-xl border-l border-gray-100 flex-col shrink-0 flex-none z-20 shadow-2xl shadow-gray-200/20">
        ...
        <div className="flex-1 px-6 py-6 flex flex-col gap-3">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 mb-2">تعلم اللغة الفرنسية</p>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="active-pill-french"
                    className="absolute inset-0 bg-blue-600 -z-10 rounded-[1.5rem]"
                  />
                )}
              </Link>
            )
        ...
        <div className="p-6 space-y-6">
          <Link 
            to="/languages"
            className="group flex items-center gap-4 px-6 py-4 rounded-2xl font-black text-sm text-gray-400 hover:bg-blue-50 hover:text-blue-600 transition-all w-full"
          >
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.path} 
              to={item.path} 
              className={`flex flex-col items-center gap-1 px-2 py-1 rounded-xl transition-all flex-shrink-0 ${isActive ? 'text-blue-600' : 'text-gray-400'}`}
            >
              <div className={`p-2 rounded-xl transition-all ${isActive ? 'bg-blue-600/10' : ''}`}>
                <Icon size={18} />
              </div>
