...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const levelUp = localStorage.getItem('english_level_up');
    if (levelUp === 'true') {
      const level = parseInt(localStorage.getItem('english_new_level') || '0', 10);
      setNewLevel(level);
      setShowLevelUp(true);
      localStorage.removeItem('english_level_up');
      localStorage.removeItem('english_new_level');
    }
  }, [location.pathname]);

  const navItems = [
    { name: 'Adventure Map', path: '/english', icon: Map },
    { name: 'Pronunciation', path: '/english/pronunciation', icon: Mic },
    { name: 'Conversations', path: '/english/conversations', icon: BookOpen },
    { name: 'Quizzes', path: '/english/quizzes', icon: Lightbulb },
    { name: 'Profile', path: '/english/profile', icon: User },
  ];

  return (
    <div className="min-h-screen bg-[#FDFCFB] flex flex-col md:flex-row overflow-hidden font-sans" dir="ltr">
      <LevelUpModal isOpen={showLevelUp} level={newLevel} onClose={() => setShowLevelUp(false)} />
      {/* Background Decor */}
      <div className="fixed top-0 left-0 w-[500px] h-[500px] bg-green-500/10 rounded-full blur-[120px] pointer-events-none -z-10" />
      <div className="fixed bottom-0 right-0 w-[500px] h-[500px] bg-green-500/5 rounded-full blur-[120px] pointer-events-none -z-10" />
      ...
      <nav className="hidden md:flex w-56 bg-white/80 backdrop-blur-xl border-r border-gray-100 flex-col shrink-0 flex-none z-20 shadow-2xl shadow-gray-200/20">
        ...
        <div className="flex-1 px-6 py-6 flex flex-col gap-3">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 mb-2">Learn English</p>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="active-pill-english"
                    className="absolute inset-0 bg-green-600 -z-10 rounded-[1.5rem]"
                  />
                )}
              </Link>
            )
        ...
        <div className="p-6 space-y-6 border-t border-gray-100">
          <Link 
            to="/languages"
            className="group flex items-center gap-4 px-6 py-4 rounded-2xl font-black text-sm text-gray-400 hover:bg-green-50 hover:text-green-600 transition-all w-full"
          >
      ...
      <div className="md:hidden bg-white border-b border-gray-100 p-4 flex items-center justify-between sticky top-0 z-30">
        ...
        <Link to="/languages" className="text-xs font-bold text-gray-500 hover:text-green-600 transition-colors">
          Change Language
        </Link>
      </div>
      ...
      <main className="flex-1 relative h-[calc(100vh-140px)] md:h-screen overflow-y-auto bg-transparent">
        ...
        <div className="sticky top-0 z-40 bg-white/70 backdrop-blur-md border-b border-gray-100 p-4 flex items-center justify-between shadow-sm rounded-b-3xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 block">Adventure Map</span>
              <span className="text-sm font-black text-gray-900">Learn English</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-center">
              <span className="text-sm font-black text-gray-700">You are now learning English</span>
              <EnglishFlag className="w-8 h-6 rounded shadow-sm border border-gray-200" />
            </div>
          </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.path} 
              to={item.path} 
              className={`flex flex-col items-center gap-1 px-2 py-1 rounded-xl transition-all flex-shrink-0 ${isActive ? 'text-green-600' : 'text-gray-400'}`}
            >
              <div className={`p-2 rounded-xl transition-all ${isActive ? 'bg-green-600/10' : ''}`}>
                <Icon size={18} />
              </div>
