...
export default function Profile() {
  ...
  // Spanish-specific progress from localStorage
  const spanishXP = parseInt(localStorage.getItem('spanish_xp') || '0', 10);
  ...
  const spanishLevel = calculateLevel(spanishXP);
  const completedScenarios = parseInt(localStorage.getItem('spanish_completed_scenarios') || '0', 10);
  ...
  const achievements = [
    { id: 1, title: 'البداية الإسبانية', description: 'أكمل أول درس إسباني', icon: 'Sparkles', unlocked: completedLessons.length > 0 },
    { id: 2, title: 'المثابر الإسباني', description: 'حافظ على سلسلة 7 أيام', icon: 'Flame', unlocked: streak >= 7 },
    { id: 3, title: 'خبير الإسبانية', description: 'اكمل 10 دروس', icon: 'Brain', unlocked: completedLessons.length >= 10 },
    { id: 4, title: 'قارئ الإسبانية', description: 'اكمل 5 نصوص', icon: 'BookOpen', unlocked: completedScenarios >= 5 },
    { id: 5, title: 'نجم الإسبانية', description: 'اجمع 1000 نقطة خبرة', icon: 'Star', unlocked: spanishXP >= 1000 },
    { id: 6, title: 'ماستر الإسبانية', description: 'اكمل 20 درس', icon: 'Trophy', unlocked: completedLessons.length >= 20 },
  ];

  const quests = [
    { id: 1, title: 'أكمل درس إسباني واحد', progress: Math.min(completedLessons.length, 1), target: 1, completed: completedLessons.length >= 1, xpReward: 50 },
    ...
    { id: 3, title: 'اجمع 50 نقطة', progress: Math.min(spanishXP, 50), target: 50, completed: spanishXP >= 50, xpReward: 50 },
    { id: 4, title: 'حافظ على سلسلة 3 أيام', progress: Math.min(streak, 3), target: 3, completed: streak >= 3, xpReward: 50 },
    ...
    { id: 6, title: 'اجمع 100 نقطة', progress: Math.min(spanishXP, 100), target: 100, completed: spanishXP >= 100, xpReward: 100 },
    ...
    { id: 9, title: 'اجمع 200 نقطة', progress: Math.min(spanishXP, 200), target: 200, completed: spanishXP >= 200, xpReward: 150 },
    ...
    { id: 12, title: 'اجمع 300 نقطة', progress: Math.min(spanishXP, 300), target: 300, completed: spanishXP >= 300, xpReward: 200 },
    ...
    { id: 15, title: 'اجمع 500 نقطة', progress: Math.min(spanishXP, 500), target: 500, completed: spanishXP >= 500, xpReward: 300 },
    ...
    { id: 18, title: 'اجمع 750 نقطة', progress: Math.min(spanishXP, 750), target: 750, completed: spanishXP >= 750, xpReward: 400 },
    ...
    { id: 20, title: 'اجمع 1000 نقطة', progress: Math.min(spanishXP, 1000), target: 1000, completed: spanishXP >= 1000, xpReward: 500 },
    ...
    { id: 23, title: 'اجمع 1500 نقطة', progress: Math.min(spanishXP, 1500), target: 1500, completed: spanishXP >= 1500, xpReward: 600 },
    ...
    { id: 26, title: 'اجمع 2000 نقطة', progress: Math.min(spanishXP, 2000), target: 2000, completed: spanishXP >= 2000, xpReward: 750 },
    ...
    { id: 29, title: 'اجمع 3000 نقطة', progress: Math.min(spanishXP, 3000), target: 3000, completed: spanishXP >= 3000, xpReward: 900 },
    { id: 30, title: 'أصبح بطل الإسبانية!', progress: Math.min(spanishXP, 5000), target: 5000, completed: spanishXP >= 5000, xpReward: 1000 },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="w-full mb-12 mt-8">
        ...
        >
          <div className="w-24 h-24 bg-gradient-to-br from-teal-600 to-teal-400 rounded-3xl flex items-center justify-center shadow-lg shadow-teal-600/30">
          ...
          <div>
            ...
            <p className="text-gray-500 font-bold text-lg">المستوى {stats.level} - الإسبانية</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        ...
        <motion.div 
          ...
          className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-teal-400/5 rounded-bl-full -mr-8 -mt-8" />
          <Trophy className="w-12 h-12 text-teal-500 mb-2 relative z-10" />
        ...
        >
          ...
          <h3 className="text-3xl font-black text-gray-800 relative z-10">{stats.completedScenarios}</h3>
          <p className="text-gray-400 font-bold mt-1 relative z-10">نص مكتمل</p>
        </motion.div>
      </div>
      ...
      <div className="w-full mb-12">
        <div className="flex items-center gap-3 mb-6 px-2">
          ...
          <span className="ml-auto bg-teal-600/10 text-teal-600 px-4 py-2 rounded-xl text-sm font-bold">
              ...
              <div className="flex-1 ml-4">
                ...
                <div className="flex items-center gap-4 mt-2">
                  <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <motion.div 
                      className={`h-full rounded-full ${quest.completed ? 'bg-green-500' : 'bg-teal-600'}`}
                      initial={{ width: 0 }}
