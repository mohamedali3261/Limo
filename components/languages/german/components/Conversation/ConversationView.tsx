...
import { Volume2, ChevronRight, PlayCircle, Mic, Star, ClipboardCheck } from 'lucide-react';
...
export default function ConversationView() {
  ...
  const [mascotState, setMascotState] = useState<'idle' | 'happy' | 'sad' | 'talking' | 'thinking'>('idle');
  ...
  const playAudio = (text: string, id: number, lineIdx: number, onEnd?: () => void) => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      setMascotState('talking');
      setMascotMessage('استمع بانتباه! 👂');
      ...
      utterance.onend = () => {
        ...
        setMascotMessage('ممتاز! 🎉');
        if (onEnd) onEnd();
      };
      utterance.onerror = () => {
        setActiveSpeechId(null);
        setMascotState('sad');
        setMascotMessage('حدث خطأ! 😞');
        if (onEnd) onEnd();
      };
  ...
  const playEntireConversation = () => {
    ...
    const playNext = () => {
      ...
      if (currentLine < conversation.dialog.length) {
        ...
        playAudio(line.german, line.id, currentLine, () => {
          currentLine++;
          ...
          return (
            ...
            >
              <div className={`flex flex-col max-w-[95%] md:max-w-[85%] ${isLeft ? 'items-start' : 'items-end'}`}>
                ...
                {/* Chat Bubble */}
                <button
                  onClick={() => playAudio(line.german, line.id, idx)}
