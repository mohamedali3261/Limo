...
import { Star, Check, Lock, Cloud, TreePine, Map as MapIcon, Flag, Sparkles, Award, Flame, GraduationCap, Trophy } from 'lucide-react';
...
export default function AdventureMap() {
  ...
  // Show all units but mark locked levels
  const filteredUnits = useMemo(() => {
    ...
    return courseUnits.map(unit => ({
      ...
      levels: unit.levels.map(level => {
        // If level has no difficulty, show it as unlocked
        if (!level.difficulty) return level;
        
        const levelIndex = levelOrder.indexOf(level.difficulty);
        const isAccessible = levelIndex >= currentIndex;
        
        // Mark as locked if not accessible
        if (!isAccessible) {
          return {
            ...level,
            isLocked: true
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-40 bg-white/70 backdrop-blur-md border-b border-gray-100 p-4 flex items-center justify-between shadow-sm rounded-b-3xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-sm font-black text-gray-900">تعلم اللغة الألمانية</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-center">
            <span className="text-sm font-black text-gray-700">أنت الآن تدرس اللغة الألمانية</span>
        ...
        <div key={unit.id} className="w-full mb-8 relative z-10 max-w-[450px]">
          ...
          <div className="flex flex-col items-center w-full relative">
            {unit.levels.map((level, i) => {
              ...
              const isLastInCourse = unitIndex === filteredUnits.length - 1 && i === unit.levels.length - 1;
      ...
      {/* Epic Finale Marker */}
      <div className="flex flex-col items-center pt-20 relative z-10 mt-12 max-w-[450px]">
        <motion.div 
