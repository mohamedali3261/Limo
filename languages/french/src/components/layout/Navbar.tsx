  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full backdrop-blur-xl bg-white/80 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-2">
            <NavLink to="/" icon={Map}>مسار التعلم</NavLink>
            <NavLink to="/pronunciation" icon={Mic}>محادثات تفاعلية</NavLink>
            <NavLink to="/phrases" icon={BookOpen}>المحادثة</NavLink>
            <NavLink to="/quizzes" icon={Lightbulb}>اختبارات</NavLink>
          </nav>
        </div>
      </header>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 px-2 pb-safe pt-2 flex justify-around items-center h-16 pointer-events-auto">
        <MobNavLink to="/" icon={Map} label="الرحلة" />
        <MobNavLink to="/pronunciation" icon={Mic} label="محادثة صوتية" />
        <MobNavLink to="/phrases" icon={BookOpen} label="جمل شائعة" />
        <MobNavLink to="/quizzes" icon={Lightbulb} label="اختبارات" />
      </nav>
    </>
  );
};
